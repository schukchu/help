...
def reshulya():
    command = int(input(" If you не говорите по-английски, введите текст в кавычках (да, я дуралей)"
                        ...
                        " If you wish to translate to bits, enter 42"))
    if command == 42:
        print("To translate from megabytes to bits, enter 5"
              "To translate from kilobytes to bits, enter 4"
              "To translate from bytes to bits, enter 3")
        trans = int(input())
        if trans == 5:
            her = float(input())
            print("You've got " + str(her * 1024 * 1024 * 8) + "bits from megabytes")
        if trans == 4:
            her = float(input())
            print("You've got " + str(her * 1024 * 8) + "bits from kilobytes")
        if trans == 3:
            her = float(input())
            print("You've got " + str(her * 8) + "bits from bits")
